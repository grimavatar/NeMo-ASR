...
class NeMoASR:
    def __init__(self, model_name: str = "nvidia/parakeet-tdt-0.6b-v3", target_sr: int = 16000) -> tuple[list[str], list[list[dict]]]:
    ...
    def transcribe(self, audio: str | Path | tuple[np.ndarray, int] | list, alignment_level: str = "word"):
        ...
        audio = [self.load_audio(e, target_sr) for e in audio]
    ...
    def load_audio(self, audio: str | Path | tuple[np.ndarray, int], target_sr: int) -> np.ndarray:
